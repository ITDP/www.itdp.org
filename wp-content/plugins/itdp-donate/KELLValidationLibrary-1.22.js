//KELL Validation
// v 1.22
//place immediatly after jquery library import
...
function checkForm() {
    formOkToSubmit = true;
    j$('.required').each(function(index) {
        if (!CheckField(this)) {
...
function MakeStateDropdown(labelName){
    ...
    j$(p).html('<label for="BillingStateProvince">'+labelName+':</label>');
    j$(p).append('<select name="BillingStateProvince" size="1" id="BillingStateProvince" ></select>');
}
function MakeStateOther(){
     var p = j$('#BillingStateProvince').parent();
    j$(p).html('<label for="BillingStateProvince">State / Province:</label>');
